...
void SetFatalRebootTarget() {
    init_fatal_reboot_target = "recovery";
}
...
void __attribute__((noreturn)) InitFatalReboot() {
    ...
    if (pid == -1) {
        ...
        LOG(INFO) << "Couldn't fork, rebooting into recovery;";
        ...
        LOG(INFO) << "pid == 0, rebooting into recovery;";
        sleep(5);
        RebootSystem(ANDROID_RB_RESTART2, init_fatal_reboot_target);
    }
    ...
    LOG(INFO) << "pid \{-1,0}, rebooting into recovery;";
    RebootSystem(ANDROID_RB_RESTART2, init_fatal_reboot_target);
}
...
void InstallRebootSignalHandlers() {
    ...
    action.sa_handler = [](int signal) {
        ...
        // development builds.
	LOG(INFO) << "rebooting into recovery (\"action.sa_handler\") signal = " << signal << ";";
        InitFatalReboot();
    };
